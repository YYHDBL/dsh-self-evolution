...
const boot = (home, state, port, extra = []) => {
  try { sh(TSX, [START, '--home', home, '--state', state, '--port', String(port), ...extra]); return { code: 0 } }
...
// 1. healthy baseline boot
{
  ...
  results.push({ case: '1-healthy', exit: r.code, pass: r.code === 0 })
}
...
// 2. pre-spawn lock recovery (h): dead pid cleaned; live pid aborts
{
  ...
  const rLive = (() => { try { sh(TSX, [START, '--home', home, '--state', state, '--port', '4620']); return 0 } catch (e) { return e.status ?? 1 } })()
  child.kill('SIGKILL')
  // now both dead → cleaned → boots healthy
  const rDead = (() => { try { sh(TSX, [START, '--home', home, '--state', state, '--port', '4621']); return 0 } catch (e) { return e.status ?? 1 } })()
  results.push({ case: '2-locks', liveLockExit: rLive, liveAborts: rLive === 2, deadCleanedBoots: rDead === 0, pass: rLive === 2 && rDead === 0 })
}
...
// 3. corrupt control (e2): empty-set start, paused marker, safety config preserved
{
  ...
  const r = (() => { try { sh(TSX, [START, '--home', home, '--state', state, '--port', '4622']); return { code: 0 } } catch (e) { return { code: e.status ?? 1 } } })()
  ...
  results.push({ case: '3-corrupt-empty-set', exit: r.code, paused: paused.state === 'paused', uploadOffPreserved: dump.includes('enabled: false'), pass: r.code === 0 && paused.state === 'paused' && dump.includes('enabled: false') })
}
...
// 4. trial candidate faulty (b/b2/c2): revoke-trial, baseline untouched, retry healthy
{
  ...
  const r = (() => { try { sh(TSX, [START, '--home', home, '--state', state, '--port', '4623']); return { code: 0 } } catch (e) { return { code: e.status ?? 1 } } })()
  const after = controlMod.readControl(state)
  results.push({ case: '4-revoke-trial', exit: r.code, baselineUntouched: after.baseline.hash === before.baseline.hash, trialInvalidated: after.activeTrial?.status === 'invalidated', pass: r.code === 0 && after.baseline.hash === before.baseline.hash && after.activeTrial?.status === 'invalidated' })
}
...
// 5. formal baseline faulty (a/c): recover-baseline, quarantine, retry healthy
{
  ...
  const r = (() => { try { sh(TSX, [START, '--home', home, '--state', state, '--port', '4624']); return { code: 0 } } catch (e) { return { code: e.status ?? 1, out: String(e.stdout ?? '') } } })()
  const after = controlMod.readControl(state)
  results.push({ case: '5-recover-baseline', exit: r.code, quarantined: after.quarantine.some(q => q.hash === 'B-bad'), baselineBackTo: after.baseline.hash, pass: r.code === 0 && after.quarantine.some(q => q.hash === 'B-bad') && after.baseline.hash === 'A-empty' })
}
...
// 6. forged health file with wrong bootId (g): ignored, real health accepted
{
  ...
  const r = (() => { try { sh(TSX, [START, '--home', home, '--state', state, '--port', '4625']); return { code: 0 } } catch (e) { return { code: e.status ?? 1 } } })()
  const accepted = JSON.parse(readFileSync(join(state, 'launcher-health.json'), 'utf8'))
  results.push({ case: '6-forged-health-ignored', exit: r.code, finalHealthNotForged: accepted.bootId !== 'forged-boot-id', pass: r.code === 0 && accepted.bootId !== 'forged-boot-id' })
}

// 7. unknown exit after healthy start (f/c): diagnostics kept, not auto-attributed
{
  const { home, state } = fixture('unknown-exit')
  const launcher = spawn(TSX, [START, '--home', home, '--state', state, '--port', '4626', '--hold'], { cwd: ROOT, stdio: ['ignore', 'pipe', 'pipe'] })
  let out = ''
  ...
  const webPids = execFileSync('pgrep', ['-f', `port 4626`]).toString().trim().split('\n').map(Number).filter(p => p !== launcher.pid)
  ...
  results.push({ case: '7-unknown-exit', launcherExit: code, record: rec ? { autoAttributed: rec.autoAttributed } : null, pass: code !== 0 && rec !== null && rec.autoAttributed === false })
}
